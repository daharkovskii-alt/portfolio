...
float fbm(vec2 p){
  p *= 1.1;
  float f = 0.0; float amp = 0.5 * uNoiseAmp;
...
export default function FaultyTerminal({
  ...
  const resolvedDpr = useMemo(() => dpr ?? (typeof window !== "undefined" ? Math.min(window.devicePixelRatio || 1, 2) : 1), [dpr]);
  ...
  }, [resolvedDpr, pause, timeScale, scale, gridMul, digitSize, scanlineIntensity, glitchAmount,
      flickerAmount, noiseAmp, chromaticAberration, ditherVal, curvature, tintVec, mouseReact,
      mouseStrength, pageLoadAnimation, brightness, handleMouseMove]);
